...
export const onsenSlice = createSlice({
  ...
  reducers: {
    setOnsenModalView: (
      state,
      action: PayloadAction<OnsenModalView | undefined>,
    ) => {
    ...
    setOnsenModalState: (
      state,
      action: PayloadAction<{ view: OnsenModalView; open: boolean }>,
    ) => {
...
export const { setOnsenModalView, setOnsenModalOpen, setOnsenModalState } =
  onsenSlice.actions
